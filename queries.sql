...
SELECT title FROM movies WHERE synopsis ILIKE '%thrilling%' ORDER BY title;
...
SELECT title, year, rating FROM movies WHERE genre_id = 17 AND year > 1979 AND year < 1990 ORDER BY year;
...
SELECT genres.name, movies.title, movies.year --, cast_members.character
FROM cast_members
JOIN actors ON cast_members.actor_id = actors.id
JOIN movies ON cast_members.movie_id = movies.id
...
JOIN genres ON movies.genre_id = genres.id
WHERE genres.name = 'Horror' ORDER BY movies.year LIMIT 5;
